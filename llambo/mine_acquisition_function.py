...
Hyperparameter configuration: {Q}"""
            ...
            if reference_configs is not None:
                prefix += "Each performance (S) is variable through sampling and estimated in terms of mean (S_avg), minimum (S_min), and maximum (S_max). Here are some collected samples:\n"
                cfg = reference_configs['sampled']
                ...
                if 'unsampled' in reference_configs:
                    prefix += "Here are some predictions of uncollected options without observed performances:\n"
                    cfg = reference_configs['unsampled']
                    for i in range(len(cfg)):
                        features = []
                        for k in cfg:
                            features.append(f'{k}: {cfg[k].iloc[i]:.{n_dp}}')
                        features = '- ' + ', '.join(features) + "\n"
                        prefix += features
            #         prefix += "Hyperparameter configuration: "
            #         features = []
            #         for k in keys:
            #             features.append(f'{k} is {reference_configs[k].iloc[i]:.{n_dp}}')
            #         prefix += ', '.join(features) + '\n'
            #     gp_cand_point, baseline = gp_candidate_point
            #     for i in range(len(gp_cand_point)):
            #         features = []
            #         for k in keys:
            #             if not 'estimate' in k:
            #                 features.append(f'{k} is {gp_cand_point[k].iloc[i]:.{n_dp}}')
            #         features = ', '.join(features)
...
Hyperparameter configuration:"""
    ...
    def get_candidate_points(self, observed_configs, observed_fvals, reference_configs=None,
        ...
        if self.lower_is_better:
            ...
            if desired_fval <= .00001:  # score can't be negative
        ...
        prompt_templates, query_templates = self._gen_prompt_tempates_acquisitions(observed_configs, observed_fvals, desired_fval, reference_configs, n_prompts=self.n_templates, use_context=use_context, use_feature_semantics=use_feature_semantics, shuffle_features=self.shuffle_features)
