...
from llambo.mine_generative_sm_utils import gen_prompt_tempates
...
class LLM_GEN_SM:
    def __init__(self, task_context, n_gens, lower_is_better, top_pct,
        ...
        if rate_limiter is None:
            ...
            self.rate_limiter = RateLimiter(max_tokens=150000, time_frame=60, max_requests=2900)
        else:
    ...
    async def _evaluate_candidate_points(self, observed_configs, observed_fvals, candidate_configs, reference_configs=None):
        ...
        all_prompt_templates, query_examples = gen_prompt_tempates(self.task_context, observed_configs, observed_fvals, candidate_configs, 
                                                                   self.lower_is_better, self.top_pct, 
                                                                   reference_configs=reference_configs, n_prompts=self.n_templates)
    ...
    def select_query_point(self, observed_configs, observed_fvals, candidate_configs, reference_configs=None, return_raw_preds=False):
        ...
        pred_probs, cost, time_taken = asyncio.run(self._evaluate_candidate_points(observed_configs, observed_fvals, candidate_configs, reference_configs=reference_configs))
