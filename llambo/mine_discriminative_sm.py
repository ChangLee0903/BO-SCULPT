...
from llambo.mine_discriminative_sm_utils import gen_prompt_tempates
...
class LLM_DIS_SM:
    def __init__(self, task_context, n_gens, lower_is_better, 
        ...
        if rate_limiter is None:
            ...
            self.rate_limiter = RateLimiter(max_tokens=200000, time_frame=60, max_requests=2900)
        else:
    ...
    async def _evaluate_candidate_points(self, observed_configs, observed_fvals, candidate_configs, reference_configs=None,
        ...
        all_prompt_templates, query_examples = gen_prompt_tempates(self.task_context, observed_configs, observed_fvals, candidate_configs, 
                                                                    reference_configs=reference_configs, n_prompts=self.n_templates, bootstrapping=self.bootstrapping,
    ...
    def select_query_point(self, observed_configs, observed_fvals, candidate_configs, reference_configs=None):
        ...
        y_mean, y_std, cost, time_taken = asyncio.run(self._evaluate_candidate_points(observed_configs, observed_fvals, candidate_configs, reference_configs))
        if self.lower_is_better:
            best_fval = np.min(observed_fvals.to_numpy())
            delta = -1*(y_mean - best_fval)
        else:
