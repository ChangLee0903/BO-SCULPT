...
from llambo.glambo import GLLAMBO
from bogp.bogp import BOGP
...
task_context = {
    ...
    'hyperparameter_constraints': {
        ...
        # 'min_impurity_decrease': ['float', 'linear', [0.0, 3.0]], 
...
class DummyExpRunner:
    def __init__(self, task_context, func, bounds=None, seed=5566):
        self.seed = seed
    ...
    def generate_initialization(self, n_samples):
        ...
        '''
        ndim = self.bounds.shape[-1]
        init_configs = []
        # Read from fixed initialization points (all baselines see same init points)
        for n in range(n_samples):
            features = {}
            for nd in range(ndim):
                # features[f'X0'] = random.uniform(self.bounds[0, nd].item(), self.bounds[1, nd].item())
                features['X0'] = init_list[self.seed - 1][n]['X0']
...
if __name__ == '__main__':
    ...
    tot_llm_cost = 0
    logger.info('='*200)
    logger.info(f'Executing LLAMBO ({sm_mode} | top_pct: {top_pct})...')
    logger.info(f'Task context: {task_context}')

    d = 1
    bounds = torch.stack([-torch.ones(d)*0, torch.ones(d)*3]).to(torch.double)
    # seed = random.choice(list(range(5566)))
    seed = int(args.out_name[-1])
    benchmark = DummyExpRunner(task_context, f, bounds, seed)
    
    # instantiate LLAMBO
    # llambo = LLAMBO(task_context, sm_mode, n_candidates=10, n_templates=2, n_gens=10, prompt_setting='partial_context',
    ...
    # llambo = BOGP(task_context, n_initial_samples=15, n_trials=25, init_f=benchmark.generate_initialization,
    #                 bbox_eval_f=benchmark.evaluate_point)
    llambo = GLLAMBO(task_context, sm_mode, n_candidates=10, n_templates=2, n_gens=10, prompt_setting='partial_context',
                    alpha=0.1, n_initial_samples=15, n_trials=25, init_f=benchmark.generate_initialization,
                    bbox_eval_f=benchmark.evaluate_point, chat_engine=chat_engine, top_pct=top_pct)
